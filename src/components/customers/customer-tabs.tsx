...
function OverviewTab({ customer }: { customer: CustomerWithRelations }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 space-y-4">
      <h3 className="font-medium">פרטים כלליים</h3>
      <div className="grid grid-cols-2 gap-4 text-sm">
        <div>
          <span className="text-gray-400">מוצרים שרכש</span>
      ...
      {customer.meetings.length > 0 && (
        <div>
          <h4 className="text-sm font-medium text-gray-500 mt-4 mb-2">פגישות</h4>
            ...
            {customer.meetings.slice(0, 5).map(m => {
              ...
              return (
                <div key={m.id} className="flex items-center justify-between text-sm py-1.5 border-b border-gray-50">
                  ...
                  <span className="text-gray-400">{new Date(m.date).toLocaleDateString("he-IL")}</span>
                </div>
              );
            })}
          </div>
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="font-medium flex items-center gap-2"><CreditCard size={18} /> סיכום כספי</h3>
      ...
      {customer.transactions.length > 0 && (
        <div className="space-y-2">
          {customer.transactions.map(t => (
            <div key={t.id} className="flex items-center justify-between text-sm py-2 border-b border-gray-50">
              <div>
              ...
              <div className="text-left">
                ...
                <div className="text-xs text-gray-400">{new Date(t.date).toLocaleDateString("he-IL")}</div>
              </div>
            </div>
          ))}
        </div>
...
function FilesTab({ files }: { files: FileRecord[] }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
      <h3 className="font-medium flex items-center gap-2 mb-3"><FileText size={18} /> קבצים</h3>
      {files.length === 0 ? (
        <p className="text-sm text-gray-400">אין קבצים</p>
      ) : (
        <div className="space-y-2">
          {files.map(f => (
            <a key={f.id} href={f.url} target="_blank" className="flex items-center justify-between text-sm py-2 border-b border-gray-50 hover:bg-gray-50 rounded px-2 -mx-2">
              <div>
...
function QuestionnaireTab({ events }: { events: FunnelEvent[] }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
      <h3 className="font-medium flex items-center gap-2 mb-3"><ClipboardList size={18} /> שאלון התאמה</h3>
      {!questionnaireEvent ? (
        <p className="text-sm text-gray-400">לא מילא שאלון</p>
      ) : (
...
function NotesTab({ notes, customerId }: { notes: Note[]; customerId: string }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
      <h3 className="font-medium flex items-center gap-2 mb-3"><MessageSquare size={18} /> הערות</h3>
      ...
      {allNotes.length === 0 ? (
        ...
        <div className="space-y-3">
          {allNotes.map(note => (
            <div key={note.id} className="text-sm py-2 border-b border-gray-50">
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs text-gray-400">{note.author}</span>
                <span className="text-xs text-gray-400">{new Date(note.created_at).toLocaleDateString("he-IL")}</span>
              </div>
              <p>{note.content}</p>
            </div>
          ))}
