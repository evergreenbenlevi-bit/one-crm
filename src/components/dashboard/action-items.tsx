    ...
    <div className="space-y-4">
      ...
      {hotLeads.length > 0 && (
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <h3 className="text-sm font-medium text-gray-500 mb-3">🔥 לידים חמים</h3>
          <div className="space-y-3">
            {hotLeads.map(lead => (
              <div key={lead.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <div>
                  <Link href={`/leads/${lead.id}`} className="text-sm font-medium hover:text-brand-600">{lead.name}</Link>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className="text-xs text-gray-400">{statusLabels[lead.current_status] || lead.current_status}</span>
                ...
                <div className="flex items-center gap-2">
                  {lead.phone && (
                    <a href={`tel:${lead.phone}`} className="p-2 rounded-lg hover:bg-gray-50 text-gray-400 hover:text-brand-600">
                      <Phone size={16} />
                    </a>
                  )}
                  {lead.phone && (
                    <a href={`https://wa.me/972${lead.phone.replace(/^0/, "")}`} target="_blank" className="p-2 rounded-lg hover:bg-gray-50 text-gray-400 hover:text-success">
      ...
      {upcomingMeetings.length > 0 && (
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <h3 className="text-sm font-medium text-gray-500 mb-3">📅 פגישות קרובות</h3>
          <div className="space-y-3">
            {upcomingMeetings.map(meeting => (
              <div key={meeting.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <div>
                  <span className="text-sm font-medium">{meeting.customers?.name || "—"}</span>
      ...
      {endingPrograms.length > 0 && (
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100">
          <h3 className="text-sm font-medium text-gray-500 mb-3">⏰ מסיימים תוכנית בקרוב</h3>
          <div className="space-y-3">
            {endingPrograms.map(customer => (
              <div key={customer.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <Link href={`/customers/${customer.id}`} className="text-sm font-medium hover:text-brand-600">{customer.name}</Link>
