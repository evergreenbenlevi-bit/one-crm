...
export const getDashboardData = unstable_cache(
  ...
  ["dashboard-data"],
  { revalidate: CACHE_TTL, tags: ["dashboard", "leads", "transactions", "campaigns"] }
);
...
export const getTopAds = unstable_cache(
  ...
  ["top-ads"],
  { revalidate: CACHE_TTL * 5, tags: ["campaigns"] }
);

export const getHotLeads = unstable_cache(
  async () => {
    const supabase = createAdminClient();
  ...
  ["hot-leads"],
  { revalidate: CACHE_TTL, tags: ["leads"] }
);
...
export const getUpcomingMeetings = unstable_cache(
  ...
  ["upcoming-meetings"],
  { revalidate: CACHE_TTL, tags: ["meetings"] }
);

export const getEndingPrograms = unstable_cache(
  async () => {
    const supabase = createAdminClient();
  ...
  ["ending-programs"],
  { revalidate: CACHE_TTL * 10, tags: ["customers"] }
);

export const getRevenueChart = unstable_cache(
  async () => {
    const supabase = createAdminClient();
  ...
  ["revenue-chart"],
  { revalidate: CACHE_TTL * 5, tags: ["transactions"] }
);

export const getCurrentGoal = unstable_cache(
  async () => {
    const supabase = createAdminClient();
    const now = new Date();
  ...
  ["current-goal"],
  { revalidate: CACHE_TTL * 60, tags: ["goals"] }
);
